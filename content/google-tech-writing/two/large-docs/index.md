...
提供導覽與提示（signposting）能確保讀者能找到他們所需的資訊，順利解決問題。
...
請選擇能描述讀者當前任務的標題，避免使用不熟悉的術語或工具名稱作為標題。
舉例來說，假設您正在撰寫建立新網站的操作文件。為了建立網站，讀者需要初始化 Froobus framework，而為了初始化，讀者必須執行 `carambola` 指令工具。
乍看之下，您可能會想使用以下其中一個標題：

* 執行 carambola 指令
* 初始化 Froobus framework

但除非您的讀者已非常熟悉這些術語與概念，否則更直觀的標題可能會更合適，例如：*建立網站*。
...
對許多能依自己步調閱讀文件的讀者而言，學習新概念、想法與技巧是一段充實的過程。但如果一次接觸太多新概念與操作說明，可能會讓人感到負擔過重。讀者更容易接受那些在需要時逐步揭露資訊的長篇文件。以下技巧可幫助您在文件中加入漸進式揭露設計：

* 儘量在需要使用某術語或概念的指令附近再介紹它們。
* 拆解大段文字。為避免單頁中出現多個冗長段落，應視情況加入表格、圖示、清單與標題。
* 拆解長串操作步驟。若步驟繁多且複雜，應將其重組為較短的清單，逐步說明如何完成子任務。
* 從簡單的範例與指令開始，逐步加入更進階且有趣的技巧。例如，在建立表單的教學中，先說明如何處理文字回應，再介紹如何處理選擇題、圖片或其他類型的回應方式。
